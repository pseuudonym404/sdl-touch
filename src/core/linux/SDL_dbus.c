...
SDL_DBus_Init(void)
{
    int libload = LoadDBUSLibrary();

    if (!dbus.session_conn && libload != -1) {
        DBusError err;
        dbus.error_init(&err);
...
SDL_bool
SDL_DBus_BacklightInhibit(SDL_bool inhibit)
{
    DBusConnection *conn = dbus.system_conn;

    if (conn == NULL) return SDL_FALSE;
    if (inhibit && display_request != -1) return SDL_TRUE;
    if (!inhibit && display_request == -1) return SDL_TRUE;

    if (inhibit) {
        DBusMessage *msg = dbus.message_new_method_call("com.canonical.Unity.Screen", "/com/canonical/Unity/Screen", "com.canonical.Unity.Screen", "keepDisplayOn");
        if (msg != NULL) {
            DBusMessage *reply = dbus.connection_send_with_reply_and_block(conn, msg, 300, NULL);
            if (reply) {
                if (!dbus.message_get_args(reply, NULL, DBUS_TYPE_INT32, &display_request, DBUS_TYPE_INVALID)) display_request = -1;
                dbus.message_unref(reply);
            }
            dbus.message_unref(msg);
        }

        if (display_request == -1) return SDL_FALSE;
    } else {
        DBusMessage *msg = dbus.message_new_method_call("com.canonical.Unity.Screen", "/com/canonical/Unity/Screen", "com.canonical.Unity.Screen", "removeDisplayOnRequest");
        dbus.message_append_args(msg, DBUS_TYPE_INT32, &display_request, DBUS_TYPE_INVALID);
        if (msg != NULL) {
            if (dbus.connection_send(conn, msg, NULL)) dbus.connection_flush(conn);
            dbus.message_unref(msg);
        }
