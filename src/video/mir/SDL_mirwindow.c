...
MIR_CreateWindow(_THIS, SDL_Window* window)
{
    ...
    MirPixelFormat pixel_format = FindValidPixelFormat(mir_data);
    if (pixel_format == mir_pixel_format_invalid) {
        return SDL_SetError("Failed to find a valid pixel format.");
    }

    mir_window->spec = MIR_mir_connection_create_spec_for_normal_surface(mir_data->connection, window->w, window->h, pixel_format);
    MIR_mir_surface_spec_set_buffer_usage(mir_window->spec, mir_data->software ? mir_buffer_usage_software : mir_buffer_usage_hardware);
    mir_window->surface = MIR_mir_surface_create_sync(mir_window->spec);
    mir_window->stream = MIR_mir_surface_get_buffer_stream(mir_window->surface);
    ...
    if (window->flags & SDL_WINDOW_OPENGL) {
        EGLNativeWindowType egl_native_window =
                        (EGLNativeWindowType)MIR_mir_buffer_stream_get_egl_native_window(mir_window->stream);
...
MIR_DestroyWindow(_THIS, SDL_Window* window)
{
    ...
    if (mir_data) {
        ...
        MIR_mir_surface_spec_release(mir_window->spec);
